...
class KoreanScriptExtractor:
    ...
    def extract_stem(self, word):
        analyzed = self.komoran.morphs(word)
        if analyzed:
            return analyzed[0]
        return word

    def extract_verbs(self, text):
        analyzed_segment = self.komoran.pos(text)
        verbs = []
        for word, pos in analyzed_segment:
            if pos.startswith('VV'):
                verbs.append(self.extract_stem(word) + "다")  # 어간에 "다"를 붙여서 동사 원형으로 변환
        return verbs

    async def analyze_and_wikify_segment(self, segment):
        analyzed_segment = self.komoran.pos(segment['text'])
        nouns = [word for word, pos in analyzed_segment if pos.startswith('NN')]
        verbs = self.extract_verbs(segment['text'])
